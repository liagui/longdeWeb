...
export default {
  ...
  mounted() {
    if (this.transData.teacher_id.length) {
      this.teacher_idArr = this.transData.teacher_id;
      this.teacher_id = this.transData.teacher_id[0]["teacher_id"];
      this.checkTeacherindex = this.transData.teacher_id[0]["teacher_id"];
    }
    console.log(JSON.stringify(this.transData));
    this.senate_id = this.transData.senate_id;
  ...
  methods: {
    ...
    selTeacher() {
      ...
      _request.http(this, "teacher/getTeacherSearchList").then((res) => {
        ...
        console.log(this.senate_id);
        if (this.senate_id.length) {
          for (var i = 0; i < jiaowuArr.length; i++) {
            for (var j = 0; j < this.senate_id.length; j++) {
              if (
                jiaowuArr[i]["teacher_id"] == this.senate_id[j]["teacher_id"]
              ) {
    ...
    clickTeacher(item, index) {
      ...
      this.teacher_idArr = [
        { teacher_id: item.teacher_id, real_name: item.real_name },
      ];
    ...
    dialogMarksure() {
      ...
      for (var i = 0; i < selSenateArr.length; i++) {
        if (selSenateArr[i]["checked"]) {
          ...
          senateArr.push({
            teacher_id: selSenateArr[i].teacher_id,
            real_name: selSenateArr[i].real_name,
          });
        }
      }
      console.log(this.teacher_id);
      if (this.teacher_id) {
        this.$emit("markSureDialog", this.teacher_idArr, senateArr);
        // _request

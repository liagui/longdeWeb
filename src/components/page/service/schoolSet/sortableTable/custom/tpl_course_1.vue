<template>
  <div class="tpl tpl-course-1">
    <tpl-title
      v-if="blockData && blockData.optionName"
      :title="blockData.optionName"
    ></tpl-title>
    <div class="model-content" v-if="blockData && blockData.course">
      <img :src="blockData.course.cover" alt="" class="left" />
      <div class="right">
        <div class="title">{{ blockData.course.title }}</div>
        <div class="detail">
          {{ blockData.course.describe }}
        </div>
        <div class="info">
          <div class="users">
            最新加入学习：
          ...
          <div class="price">
            <span class="discount">
              ¥{{ blockData.course.pricing | toFixedNum }}
            </span>
            <span class="real-price"
              >¥{{ blockData.course.sale_price | toFixedNum }}</span
            >
          </div>
        </div>
...
.right {
  ...
  .el-button {
    ...
    box-shadow: 0px 5px 5px 0px rgba(15, 173, 252, 0.2);
  }
  .discount {

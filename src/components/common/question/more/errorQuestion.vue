<template>
  <div>
    ...
    <div class="container hidden-md-and-up phone-question-wrap">
      <van-nav-bar @click-left="clickLeft" left-arrow title="错题本">
        <template #right>
          <van-icon
            v-show="!isStar"
            name="star-o"
        ...
        <div class="padding-lr y-question">
          ...
          >
            <van-swipe-item v-for="(item,index) in dataList" :key="index">
              <Ti :data="item" @setAnswer="setAnswer" :showImportant="false"></Ti>
            </van-swipe-item>
          </van-swipe>
        </div>
        ...
        <div class="y-box">
          <PcTop>
            <div slot="left">
              ...
              <div class="y-icon y-pc-errortext"></div>
            </div>
            <div slot="center">
          ...
          <el-col :span="7">
            <div class="y-box">
              <p class="text-c font24 padding-tb20">
                您的错题： 共
                <span class="color-main bold">{{ errorLen }}</span>题
              </p>
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      this.$axios
        .post("/bank/getMyErrorExamList", param)
        .then(res => {
          this.overlay = false;
          if (res.code == 200) {
            this.dataList = [];
            this.dataList = this.formatExamList(res.data, false);
            this.total = this.dataList.length;
            this.errorLen = this.getAnswer().error;
            this.currentQuestion = this.dataList[this.current];
            if (this.dataList[0]) {
              this.isStar = this.dataList[0].is_collect == 1;
              this.currentPapers_id = this.dataList[0].papers_id;
            }
          } else {
            this.$Toast(res.msg);
          }
    ...
    change(index) {
      ...
      this.currentPapers_id = this.dataList[index].papers_id;
      this.currentQuestion = this.dataList[this.current];
    },
    starClick() {
      let param = {
        papers_id: this.currentPapers_id,
        bank_id: this.bank_id,
        subject_id: this.subject_id,

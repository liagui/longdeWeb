<template>
  <div>
    ...
    <div class="container hidden-md-and-up phone-question-wrap">
      <van-nav-bar @click-left="clickLeft" left-arrow title="收藏的题">
        <template #right>
          <!-- <van-icon v-show="!isStar" name="star-o" size="18" is-link @click="starClick" color="#333" /> -->
          <van-icon name="star" size="18" is-link @click="starClick" color="#FF7939" />
        </template>
      </van-nav-bar>
        ...
        <div class="y-box">
          <PcTop>
            <div slot="left">
              ...
              <div class="y-icon y-pc-mycollect"></div>
            </div>
            <div slot="center">
          ...
          <el-col :span="7">
            <div class="y-box">
              <p class="text-c font24 padding-tb20">
                您的收藏： 共
                <span class="color-main bold">{{ total }}</span>题
              </p>
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      this.$axios.post("/bank/getMyCollectExamList", param).then(res => {
        if (res.code == 200) {
          this.dataList = [];
    ...
    starClick() {
      ...
      this.$axios.post("/bank/doCollectQuestion", param).then(res => {
        if (res.code == 200) {
          ...
          this.dataList.splice(this.current, 1);
          this.total = this.dataList.length;
          if (this.current > 0) {
